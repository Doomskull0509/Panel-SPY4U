...
$query = mysqli_query($conn,"SELECT * FROM users_data WHERE usuario = '$user'");
...
$queryselect 	= mysqli_query($conn,"SELECT name FROM users_data WHERE usuario = '$user'");
...
{
	echo "<script> alert('Acceso Exitoso, bienvenido (a)');window.location= 'panel.html' </script>";
}
else if ($nr == 0) 
{

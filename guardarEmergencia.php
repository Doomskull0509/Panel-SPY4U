...
// Verificar que se recibieron archivos
 if(!empty($_FILES['image-Emergencia'])) 
 {

     // No es necesario contar
     // Recorrer el arreglo de nombres por índice
     for($i = 0; $i < count($_FILES['image-Emergencia']['name']); $i ++)
      {
         // Comprobar que no hay error en el archivo, incluir índice
         if ($_FILES["image-Emergencia"]["error"][$i] == 0)
          {
             // Obtener ruta del archivo temporal
             $tempFileName = $_FILES["image-Emergencia"]["tmp_name"][$i];
             // Obtener nombre real
             $fileName = $_FILES["image-Emergencia"]["name"][$i];
  ...
  $guardarSQL = "INSERT INTO reg_emergencia (Fecha,Hora,Cliente,Sucursal,Categoria,Descripcion,Observaciones,imagen,operador) VALUES ('$fechax','$horax','$cliente','$sucursal','$categoria','$descripcion','$observaciones','$imgContent','$operador');";
...
//------------------------------------------------------------------------------------------------------Termina Adjuntar imagen
    ...
    $mail->Subject = "Reporte de Emergencia - ".$cliente." - ".$sucursal." - [".$fechax."]";
    ...
    $message1 .= "<tr style='color:rgb(0, 0, 0)' bgcolor='#F81818'>";  
    $message1 .= "<td colspan='4'>";  
    $message1 .= "<h1><b>";  
    $message1 .= "<center>REPORTE DE EMERGENCIA</center>";  
    $message1 .= "</b></h1>";  
    $message1 .= "</td>";  
    $message1 .= "</tr>";  
  ...
  {
    $mail->send();
    echo "<script> alert('Registro correcto');window.location='registro_Emergencia.html'  </script>";
  }
  else
  {
    echo "<script> alert('Error al guardar en DB');window.location='registro_Emergencia.html'  </script>";
